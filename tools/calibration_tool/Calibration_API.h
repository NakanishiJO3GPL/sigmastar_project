...
extern "C"
{
    ...
    #define ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM (4)
    #define ISP_AIBNR_ISO_NODE_MAX_NUM (20)
    #define ISP_AIBNR_UINT8_SCALE 255
    #define ISP_AIBNR_INT16_SCALE 32767
    #define MAX_AIBNR_GAIN_NODE             (7)
    ...
    typedef enum
    {
        ...
        eCHIP_INFO_ID_PRETZEL  = 0x03, // I5
        eCHIP_INFO_ID_MACARON  = 0x04, // I6
        eCHIP_INFO_ID_PUDDING  = 0x05, // I6E
        eCHIP_INFO_ID_ISPAHAN  = 0x06, // I6B0
        eCHIP_INFO_ID_IKAYAKI  = 0x07, // P3
        eCHIP_INFO_ID_MUFFIN   = 0x08, // I7
        eCHIP_INFO_ID_MARUKO   = 0x09, // I6C
        eCHIP_INFO_ID_SOUFFLE  = 0x0A, // I6F
        eCHIP_INFO_ID_IFORD    = 0x0B, // I6DW
        eCHIP_INFO_ID_IFADO    = 0x0C, // I6DC
        eCHIP_INFO_ID_IFACKEL  = 0x0D, // I6CP
        eCHIP_INFO_ID_PCUPID   = 0x0E, // P3P
        eCHIP_INFO_ID_JAGUAR1  = 0x0F, // J1
        eCHIP_INFO_ID_MERCURY5 = 0x10,
        eCHIP_INFO_ID_TIRAMISU = 0x11, // M6
        eCHIP_INFO_ID_MOCHI    = 0x13, // M6P
        eCHIP_INFO_ID_OPERA    = 0x14, // P5
        eCHIP_INFO_ID_IFLIEGEN = 0x15, // I6G
        eCHIP_INFO_ID_IHALO    = 0x16, // H1
        eCHIP_INFO_ID_MHERA    = 0x17, // M8
    ...
    typedef struct
    {
		...
		ALSC_HARDWARE_LIMIT_MODE uValueRangeHandlingMode;
    ...
    typedef struct
    {
        ...
        u8  hdr_mode;                          // 0 : Linear Mode, 1 : HDR Mode, 2 : HCG Mode
        u16 hdr_awb_gain[3];

        u32 to_12_log_in[TO_12_LOG_NUM];
        u32 to_12_log_out[TO_12_LOG_NUM];
        u8  to_12_log_valid_len;
    ...
    typedef struct
    {
        char  *darkfoldername_path[ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM];
        char  *colorcheckerfoldername_path[ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM];
        u8  uOpMode;   // 0: Gernerate 1: ReGenerate 2: Validate
        u8 ROINum;
        u8  uRangeNum;
        u8  uNodeNum;
        u8  uPreGainNode[MAX_AIBNR_GAIN_NODE];
        u16 u2SensorAgain;
        u16 u2IgnoreNode[ISP_AIBNR_ISO_NODE_MAX_NUM];
        u16 IsoThd[2];
        ...
        u32 u4TotalGainNode[MAX_AIBNR_GAIN_NODE];
        float KSCurveNode[ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM-1];
        float MeanThd0[3];
        float MeanThd1[3];
        float IgnoreNode[ISP_AIBNR_ISO_NODE_MAX_NUM];
        float Output_BlackISO[ISP_AIBNR_ISO_NODE_MAX_NUM];
        float Output_BlackS[ISP_AIBNR_ISO_NODE_MAX_NUM];
        float Output_ColorISO[ISP_AIBNR_ISO_NODE_MAX_NUM];
        float Output_ColorK[ISP_AIBNR_ISO_NODE_MAX_NUM];
        float Output_OB[ISP_AIBNR_ISO_NODE_MAX_NUM];
        float Output_KA[ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM];
        float Output_KB[ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM];
        float Output_SA[ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM];
        float Output_SB[ISP_AIBNR_KS_CURVE_RANGE_MAX_NUM];
    ...
    typedef struct
    {
        ...
        u8  uDecomp_input_bits;
        u8  uDecomp_output_bits;
        u8  uValueRangeHandlingMode;
        u8  uSplitMode;
        u8  uSplitCol;
        u8  uSplitRow;
        u16 u2SplitROITopLeftX[20];
        u16 u2SplitROITopLeftY[20];
        u16 u2SplitROIWidth[20];
        u16 u2SplitROIHeight[20];
        u16 u2ImageWidth;
        u16 u2ImageHeight;
        u16 u2ImageClipX;
        u16 u2ImageClipY;
    ...
    typedef struct
    {
        ...
        f32 MeanThd0[3];
        f32 MeanThd1[3];
        f32 IgnoreNode[ISP_AIBNR_ISO_NODE_MAX_NUM];
    } AINR_PLUGIN_CTRL;
     ...
     */
    DLLIMPORT s32 DoCalibrationAINR(calibration_handle *handle, void *pData, char pStrDarkFolderName[][FILENAME_PATH_MAX], char pStrColorCheckerFolderName[][FILENAME_PATH_MAX], float CurveNode[],
        float BlackISO[], float BlackS[], float ColorISO[], float ColorK[], float OB[], float As[], float Bs[], float Ak[], float Bk[],
        unsigned char node_number, unsigned char preGainNode[], unsigned long totalGainNode[], unsigned short sensor_again,
        float validate_current_iso, int* validate_num, float validate_mean[], float validate_var[], float *validate_k, float *validate_s, float *mean_error, float *max_error, float *fit_error);

    DLLIMPORT s32 LoadAIBNRCaliData(char* pStrCaliDataFileName, float CurveNode[], float As[], float Bs[], float Ak[], float Bk[]);
