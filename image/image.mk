...
%_$(FLASH_TYPE)_gz_fsimage:
	...
	$(PROJ_ROOT)/kbuild/$(KERNEL_VERSION)/scripts/mkimage -A arm -O linux -C none -a $(INITRAMFSLOADADDR) -e $(INITRAMFSLOADADDR) -n "ramdisk" -d $(IMAGEDIR)/ramdisk.img $(IMAGEDIR)/u-ramdisk.img;
	@if [ "$($(patsubst %_$(FLASH_TYPE)_gz_fsimage,%,$@)$(OPTIONS))" = "gz" ]; then	\
		cp -vf $(IMAGEDIR)/ramdisk.img $(IMAGEDIR)/initramfs; \
		gzip -n -9 -f $(IMAGEDIR)/initramfs;\
		rm -rf $(IMAGEDIR)/initramfs;\
	else \
		lz4 -f -9 -l $(IMAGEDIR)/ramdisk.img $(IMAGEDIR)/ramdisk.lz4;\
		$(PROJ_ROOT)/kbuild/$(KERNEL_VERSION)/scripts/mkimage -A arm -O linux -C lz4 -a $(INITRAMFSLOADADDR) -e $(INITRAMFSLOADADDR) -n "ramdisk" -d $(IMAGEDIR)/ramdisk.lz4 $(IMAGEDIR)/initramfs.gz; \
		rm -rf $(IMAGEDIR)/ramdisk.lz4;\
	fi;
	$(call image_size_check,$(IMAGEDIR)/initramfs.gz,$($(patsubst %_$(FLASH_TYPE)_gz_fsimage,%,$@)$(PATSIZE)))
...
ipl_nofsimage ipl_cust_nofsimage iplx_nofsimage uboot_nofsimage freertos_nofsimage optee_nofsimage tf_a_nofsimage vmm_nofsimage:
	@echo [[$@]]
...
ramdisk_nofsimage:
	@echo [[$@]]
	$(PROJ_ROOT)/kbuild/$(KERNEL_VERSION)/scripts/mkimage -A arm -O linux -C none -a 0 -e 0 -n "RAMDISK" -d $($(patsubst %_nofsimage,%,$@)$(RESOURCE)) $(IMAGEDIR)/$(patsubst %_nofsimage,%,$@)
