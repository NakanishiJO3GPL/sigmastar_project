...
customer_$(PRODUCT):
	...
	# pack amigos/sample_code/mi_demo
	@if [[ "X$(DEBUG)" == "X" ]] || ([[ "X$(DEBUG)" != "X" ]] && !((( $(DEBUG) & 1 )) || (( $(DEBUG) & 256 )))); then \
		if [ $(verify_amigos) = "enable" ]; then \
			cp -rf $(LIB_DIR_PATH)/bin/amigos $(customer$(RESOURCE))/ ; \
		fi; \
		...
		if [ $(verify_ptree) = "enable" ]; then \
			cp -rf $(LIB_DIR_PATH)/bin/ptree/ $(customer$(RESOURCE))/ ; \
		fi; \
	fi;
	...
	if [ $(SIGMA_WIFI) != "no_wifi" ]; then	\
		...
		if [[ "$(PRODUCT)" == "cardv" ]] && [[ -d "$(LIB_DIR_PATH)/wifi/webserver" ]]; then \
			...
			if [ $(FLASH_TYPE) = "nor" ]; then \
				find $(customer$(RESOURCE))/wifi -name "wpa_supplicant" | xargs rm; \
			fi; \
		fi; \
	...
	if [[ "$(DUAL_OS)" != "on" || "$(CONFIG_ENABLE_POWER_SAVE_AOV)" == "y" ]]; then \
		...
		sed -i 's/sstar_mi.ko/sstar_mi.ko common_init.g_ModParamPath=\/config\/modparam.json/g' $(customer$(RESOURCE))/demo.sh; \
	fi;
