...
misc_$(PRODUCT):
	...
	if [[ "$(DUAL_OS)" = "on" || "$(PURE_RTOS)" = "on" ]]; then \
		...
		elif [ -n "$(CUST_MODPARAM)" ]; then  \
			cp -rf $(PROJ_ROOT)/board/$(CHIP)/json/$(PRODUCT)/$(CUST_MODPARAM) $(misc$(RESOURCE))/modparam.json; \
		else \
	...
	if [[ ! "$(USR_MOUNT_BLOCKS)" =~ "rofiles" ]]; then \
		if [[ "$(DLA_FIRMWARE_LIST)" != "" ]] && [[ "$(DUAL_OS)" = "on" || "$(PURE_RTOS)" = "on" ]]; then \
