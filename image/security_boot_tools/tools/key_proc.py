    ...
    try:
        if split_file is not None:
            ...
            with open(split_file, "rb") as src_file:
                ...
                for i in range(0, 2):
                    sbot_size = i*256;
                    cust_magic, cust_fileSize = struct.unpack('IH', plainText[realy_size+sbot_size+4:realy_size+sbot_size+10])
                    if 0x4E4C5049 == cust_magic:
                        break

                HDR_Ver = struct.unpack('B', plainText[realy_size+sbot_size+14:realy_size+sbot_size+15])
                realy_cust_fileSize = get_IPL_realy_size(HDR_Ver[0], cust_fileSize)
                if 0x4E4C5049 == cust_magic:
                    cust_bin = plainText[realy_size+sbot_size:realy_size+sbot_size+realy_cust_fileSize]
...
def do_secure_append(binfile, mode, anti, aeskeylen, aeskeynum, IVfile, apd_ver, dm_verity):
    magic = b"SSTARSBT"
    pend_version = apd_ver
    sstar_pend_size = 0
    ...
    if (apd_ver == 0):
        ...
        binary_data = struct.pack(">8sBBBBB3s16s", magic, pend_version, sstar_pend_size, sel_aes_key, aes_type<<1|aes_enable,
                                  anti, b"\x00" * 3, result)
    elif (apd_ver == 1):
        #print("dm_verity = ", dm_verity)
        sstar_pend_size = 512
        uuid, start_sector, num_sectors, data_block_size, hash_block_size, data_blocks, hash_start_block, hash_algorithm, root_hash, salt = dm_verity.split(',')

        uuid = uuid.replace("-", "")
        start_sector = int(start_sector)
        num_sectors = int(num_sectors)
        data_block_size = int(data_block_size)
        hash_block_size = int(hash_block_size)
        data_blocks = int(data_blocks)
        hash_start_block = int(hash_start_block)

        # print("UUID:", uuid)
        # print("Start Sector:", start_sector)
        # print("Num Sectors:", num_sectors)
        # print("Data Block Size:", data_block_size)
        # print("Hash Block Size:", hash_block_size)
        # print("Data Blocks:", data_blocks)
        # print("Hash Start Block:", hash_start_block)
        ...
        binary_data = struct.pack('<8sBBH16sIIIIIIH32s64s', magic, pend_version, 0, sstar_pend_size, binascii.unhexlify(uuid),start_sector, num_sectors, data_block_size, hash_block_size, data_blocks, hash_start_block, hash_alg,binascii.unhexlify(salt),binascii.unhexlify(root_hash))

        padding = b'\x00' * (sstar_pend_size - struct.calcsize('<8sBBH16sIIIIIIH32s64s'))
    ...
    with open(binfile, 'rb') as bin:
        ...
        with open(output_path, 'wb') as out:
            if (apd_ver == 0):
                out.write(plainText+binary_data)
            elif (apd_ver == 1):
                out.write(binary_data+padding)
...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], 'vhf:',['version', 'IV=', 'aes=', 'rsa=', 'help', 'encrypt', 'insert', 'burn', 'sign', 'exportkey', \
            'exportkey_reverse','split', 'signusb', 'keyN', 'key2sha1', 'pwd2sha1', 'mode=', 'append', 'anti=', 'aeskeylen=', 'aeskeynum=', 'append_version=', \
            'dm_verity='])
        for o, a in opts:
            if o == "-f":
        ...
        if True == encrypt:
            ...
            do_secure_append(binfile, mode, anti, aeskeylen, aeskeynum, IVfile, apd_ver, dm_verity)
        else:
            usage()
