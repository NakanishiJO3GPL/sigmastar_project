...
#OFFSET_13: len keyN and keyAES
    ...
    out.seek(18,0)
    if (len(sys.argv)>7):
        print('%18s=0x%X' % ('AESKEY_NUM', int(sys.argv[7], 16)))
        keynum=int(sys.argv[7], 16)
    if (len(sys.argv)>8):
        if(sys.argv[8]=='1'):
            print('%18s=0x%X' % ('CBC', int(sys.argv[8], 16)))
            CBC=int(sys.argv[8], 16)
    if (len(sys.argv)>6):
        if(sys.argv[6]=='1'):
            print('%18s=0x%X' % ('AES', int(sys.argv[6], 16)))
            out.write(b'%c' % (int(sys.argv[6], 16)|(keynum<<4) | CBC<<1))
            out.flush()
