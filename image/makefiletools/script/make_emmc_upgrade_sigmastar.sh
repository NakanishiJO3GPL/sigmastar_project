...
TMP_UPGRADE_FILE=TMP_SigmastarUpgradeEMMC.bin
PADDED_BIN=$TARGET_DIR/padded.bin
PAD_DUMMY_BIN=$TARGET_DIR/dummy_pad
UPGRADE_FILE=$TARGET_DIR/SigmastarUpgradeEMMC.bin
...
do
    ...
    P)
        ...
        ;;
    ?)
        echo "usage: $0 -F --> PACK ALL"
		echo "usage: $0 -P --> PACK BY PART"
        exit 1
        ;;
    esac
done
