...
linux-kernel-debug:
	...
	@if [[ -f $(KERNEL_DIR)/$(KERNEL_CONFIG) ]]; then \
		...
		#enable KASAN;\
		if (( $(DEBUG) & 2 )) || (( $(DEBUG) & 1 )); then sed -i '$$a\CONFIG_KASAN=y\nCONFIG_KASAN_INLINE=y\nCONFIG_ARM_MODULE_PLTS=y\nCONFIG_MODULES_AREA_SIZE=0x01000000' $(KERNEL_DIR)/$(KERNEL_CONFIG)_debug_defconfig; fi\
		#enable LOCKDEP;\
		...
		# get origin image config file; \
		IMAGE_CONFIG=$(IMAGE_CONFIG); \
		IMAGE_CONFIG=`echo $${IMAGE_CONFIG%%_debug}`; \
		if [[ -f "image/configs/general/$${IMAGE_CONFIG}" ]]; then \
			cp image/configs/general/$${IMAGE_CONFIG} image/configs/general/$${IMAGE_CONFIG}_debug; \
			KernPatSize=`grep '^kernel$$(PATSIZE)' image/configs/general/$(IMAGE_CONFIG)`;\
			KernPatSize=`echo $${KernPatSize##*=}`; \
			sed -i "s/^kernel\$$(PATSIZE).*=.*/kernel\$$(PATSIZE) = \$$(call sum, $${KernPatSize} $${KernPatSize})/g" image/configs/general/$${IMAGE_CONFIG}_debug; \
			CustPatSize=`grep '^customer$$(PATSIZE)' image/configs/general/$(IMAGE_CONFIG)`;\
			custPatSize=`echo $${CustPatSize##*=}`; \
			sed -i "s/^customer\$$(PATSIZE).*=.*/customer\$$(PATSIZE) = \$$(call sub, $${custPatSize}, $${KernPatSize})/g" image/configs/general/$${IMAGE_CONFIG}_debug; \
			sed -i "s/^IMAGE_CONFIG.*/IMAGE_CONFIG = $${IMAGE_CONFIG}_debug/g" configs/current.configs; \
		fi; \
		fi; \
