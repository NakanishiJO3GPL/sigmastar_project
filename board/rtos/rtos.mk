...
define modify_stacksize_for_rtos_asan
	if [ "$(DEBUG)X" != "X" ] && [ $$(($(DEBUG) & 2048)) -ne 0 ]; then \
		python $(PROJ_ROOT)/board/rtos/modify_stacksize_for_rtos_asan.py $(1) true;\
	else \
		python $(PROJ_ROOT)/board/rtos/modify_stacksize_for_rtos_asan.py $(1) false;\
	fi;
endef
...
rtos:
	...
	@if [[ -f $(RTOS_CONFIGS_DIR)/$(RTOS_CONFIG) ]]; then \
		cd $(RTOS_ROOT); $(MAKE) $(RTOS_CONFIG) || exit 1; \
		$(call modify_stacksize_for_rtos_asan, $(RTOS_ROOT)/.config ) \
...
#		ln -f $(RTOS_ROOT)/build/$(RTOS_OUT)/out/u-$(RTOS_OUT).bin $(RTOS_IMAGES_ROOT)/u-$(RTOS_OUT).bin; \
		...
		if [[ -f $(RTOS_ROOT)/build/$(RTOS_OUT)/out/u-$(RTOS_OUT).lz4 ]]; then \
			ln -f $(RTOS_ROOT)/build/$(RTOS_OUT)/out/u-$(RTOS_OUT).lz4 $(RTOS_IMAGES_ROOT)/rtos.lz4; \
		fi; \
