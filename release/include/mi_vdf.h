...
#define VDF_SUB_VERSION 09
#define MACRO_TO_STR(macro) #macro
#define VDF_VERSION_STR(major_version, sub_version)                                                            \
    ({                                                                                                         \
        char* tmp = sub_version / 100                                                                          \
                        ? "mi_vdf_version_" MACRO_TO_STR(major_version) "." MACRO_TO_STR(sub_version)          \
                        : sub_version / 10                                                                     \
                              ? "mi_vdf_version_" MACRO_TO_STR(major_version) ".0" MACRO_TO_STR(sub_version)   \
                              : "mi_vdf_version_" MACRO_TO_STR(major_version) ".00" MACRO_TO_STR(sub_version); \
        tmp;                                                                                                   \
    })
...
MI_S32 MI_VDF_ResetBackground(MI_VDF_CHANNEL VdfChn);
